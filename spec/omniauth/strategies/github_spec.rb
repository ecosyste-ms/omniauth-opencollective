...
describe OmniAuth::Strategies::OpenCollective do
  ...
  let(:enterprise) do
    OmniAuth::Strategies::OpenCollective.new('OPENCOLLECTIVE_KEY', 'OPENCOLLECTIVE_SECRET',
        {
            :client_options => {
  ...
  subject do
    OmniAuth::Strategies::OpenCollective.new({})
  end
  ...
  context '#info.urls' do
    it 'should use html_url from raw_info' do
      ...
      expect(subject.info['urls']['OpenCollective']).to eq('http://enterprise/me')
    end
  end
